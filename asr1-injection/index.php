...
$route->map('GET', '/get', function (ServerRequestInterface $request, ResponseInterface $response) {
    $params = $request->getQueryParams();

    if (isset($params['file'])) {
        serve_file($params['file']);
        exit;
    }

    $response->getBody()->write('Please specify a file with the ?file query parameter!');
    return $response;
});
